        ...
        for epoch in range(self.config.num_epochs):
            ...
            for step, data in enumerate(tqdm(self.train_data_loader)):
                ...
                if (step + 1) % self.config.logging_interval == 0:
                    ...
                    self.summary_writer.add_scalar("nsmc/train/loss", train_loss, self.global_step)
                    self.summary_writer.add_scalar("nsmc/train/accuracy", train_acc, self.global_step)
            ...
            self.summary_writer.add_scalar("nsmc/dev/loss", dev_loss, self.global_step)
            self.summary_writer.add_scalar("nsmc/dev/accuracy", dev_acc, self.global_step)
            ...
            self.summary_writer.add_scalar("nsmc/test/loss", test_loss, self.global_step)
            self.summary_writer.add_scalar("nsmc/test/accuracy", test_acc, self.global_step)
            ...
            if os.path.isfile('./run_outputs/total_log.csv') == False:
                with open ('./run_outputs/total_log.csv', 'w', newline="") as f:
                    ...
                    wr.writerow([self.begin_time, 'nsmc', self.pretrained_bert_file_name, self.config.tokenizer, self.config.batch_size, self.config.learning_rate, epoch, self.dev_result, self.test_result])
