        ...
        for epoch in range(self.config.num_epochs):
            ...
            if os.path.isfile('./run_outputs/total_log.csv') == False:
                with open ('./run_outputs/total_log.csv', 'w', newline="") as f:
                    ...
                    wr.writerow([self.begin_time, 'cola', self.pretrained_bert_file_name, self.config.tokenizer, self.config.batch_size, self.config.learning_rate, epoch, f"{self.dev_result:.4f}", f"{self.test_result:.4f}"])
