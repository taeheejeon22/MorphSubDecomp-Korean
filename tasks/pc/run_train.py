...
from tasks.pc.data_utils import load_data
from tasks.pc.dataset import PCDataset
from tasks.pc.model import PCModel
from tasks.pc.trainer import Trainer
from tokenizer import (
    # CharTokenizer,
    # JamoTokenizer,
...
def main(args):
    ...
    train_dataset = PCDataset(train_sentences, train_labels, vocab, tokenizer, config.max_sequence_length)
    ...
    dev_dataset = PCDataset(dev_sentences, dev_labels, vocab, tokenizer, config.max_sequence_length)
    dev_data_loader = DataLoader(dev_dataset, batch_size=1024)
    # Test
    logger.info("create data loader using test data")
    test_dataset = PCDataset(test_sentences, test_labels, vocab, tokenizer, config.max_sequence_length)
    ...
    model = PCModel(bert_config, config.dropout_prob)
