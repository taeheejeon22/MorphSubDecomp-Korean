        ...
        for epoch in range(self.config.num_epochs):
            ...
            for step, data in enumerate(tqdm(self.train_data_loader)):
                ...
                if (step + 1) % self.config.logging_interval == 0:
                    ...
                    self.summary_writer.add_scalar("paws/train/loss", train_loss, self.global_step)
                    self.summary_writer.add_scalar("paws/train/accuracy", train_acc, self.global_step)
            ...
            self.summary_writer.add_scalar("paws/dev/loss", dev_loss, self.global_step)
            self.summary_writer.add_scalar("paws/dev/accuracy", dev_acc, self.global_step)
            ...
            self.summary_writer.add_scalar("paws/test/loss", test_loss, self.global_step)
            self.summary_writer.add_scalar("paws/test/accuracy", test_acc, self.global_step)
            ...
            if os.path.isfile('./run_outputs/total_log.csv') == False:
                with open ('./run_outputs/total_log.csv', 'w', newline="") as f:
                    wr = csv.writer(f)
                    self.dev_result = round(dev_acc * 100, 4)
                    self.test_result = round(test_acc * 100, 4)
                    wr.writerow('time', 'task', 'model', 'tokenizer', 'batch_size', 'lr', 'epoch', 'dev', 'test')
                    wr.writerow([self.begin_time, 'paws', self.pretrained_bert_file_name, self.config.tokenizer, self.config.batch_size, self.config.learning_rate, epoch, f"{self.dev_result:.4f}", f"{self.test_result:.4f}"])
