...
from tasks.cb.config import TrainConfig
from tasks.cb.model import CBModel
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for step, data in enumerate(tqdm(self.train_data_loader)):
                ...
                if (step + 1) % self.config.logging_interval == 0:
                    ...
                    self.summary_writer.add_scalar("cb/train/loss", train_loss, self.global_step)
                    self.summary_writer.add_scalar("cb/train/accuracy", train_acc, self.global_step)
            ...
            self.summary_writer.add_scalar("cb/dev/loss", dev_loss, self.global_step)
            self.summary_writer.add_scalar("cb/dev/accuracy", dev_acc, self.global_step)
            ...
            self.summary_writer.add_scalar("cb/test/loss", test_loss, self.global_step)
            self.summary_writer.add_scalar("cb/test/accuracy", test_acc, self.global_step)
            ...
            if os.path.isfile('./run_outputs/total_log.csv') == False:
                with open ('./run_outputs/total_log.csv', 'w', newline="") as f:
                    ...
                    wr.writerow([self.begin_time, 'cb', self.pretrained_bert_file_name, self.config.tokenizer, self.config.seed, self.config.batch_size, self.config.learning_rate, epoch, self.dev_result, self.test_result])
