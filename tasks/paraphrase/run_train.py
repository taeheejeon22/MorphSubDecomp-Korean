...
from tasks.paraphrase.config import TrainConfig
from tasks.paraphrase.data_utils import load_data
from tasks.paraphrase.dataset import ParaphraseDataset
from tasks.paraphrase.model import ParaphraseModel
from tasks.paraphrase.trainer import Trainer
from tokenizer import (
    # CharTokenizer,
    # JamoTokenizer,
...
def main(args):
    ...
    train_dataset = ParaphraseDataset(train_sentences, train_labels, vocab, tokenizer, config.max_sequence_length)
    ...
    dev_dataset = ParaphraseDataset(dev_sentences, dev_labels, vocab, tokenizer, config.max_sequence_length)
    dev_data_loader = DataLoader(dev_dataset, batch_size=1024)
    # Test
    logger.info("create data loader using test data")
    test_dataset = ParaphraseDataset(test_sentences, test_labels, vocab, tokenizer, config.max_sequence_length)
    ...
    model = ParaphraseModel(bert_config, config.dropout_prob)
