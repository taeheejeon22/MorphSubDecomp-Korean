# v1: wiki extractor 대신 이기창 코드 사용
...
in_f = "../kowiki-latest-pages-articles.xml.bz2"    # 10 21 latest
out_f = "../wikiko_20211021_preprocessed_wiki.txt"

make_corpus(in_f,out_f)

##################################################
with open(out_f, "r") as f:
    all_docs = f.readlines()    # 382,468 개 문서
    all_docs = [doc[:-1] for doc in all_docs]   # 문서별 마지막 "\n" 제거
...
# for ix in tqdm(range( 100 )):
    ...
    split_text0 = kss.split_sentences(all_docs[ix])

    split_text1 = [re.sub(p_kakao, "", sent) for sent in split_text0]
...
# save as a txt
with open("../wikiko_20211021_with_preprocessing_v3.txt", "w") as f:
    f.write(all_texts)
