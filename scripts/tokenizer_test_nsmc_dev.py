...
nsmc_test = nsmc_load_data('dataset/nlu_tasks/nsmc/ratings_dev.tsv')


if __name__=='__main__':
    # p = Pool(14)

    # with open('tokenized_result/nsmc_train_spacing.csv', "w", encoding='utf-8') as f:
    ...
    with open('tokenized_result/nsmc_dev_spacing.csv', "w", encoding='utf-8') as f:
        
        show_tokenizations("뱌뵵뵤벼벼벼추퓨를 먹었다.")
        f.write('tokenizer'+'\t'+'result')
        with Pool(4) as p:
            f.write(''.join(p.map(show_tokenizations, nsmc_test)))
            # f.write('\n'.join([str(x) for x in range(1, 10000)]))
