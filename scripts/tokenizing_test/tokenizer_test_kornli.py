...
kornli_test = load_data('dataset/nlu_tasks/kornli/xnli.test.ko.tsv')

with open('tokenized_result/kornli_test.csv', "w", encoding='utf-8') as f:
    show_tokenizations("뱌뵵뵤벼벼벼추퓨를 먹었다.")
    for sent1, sent2 in zip(kornli_test[0], kornli_test[1]):
        f.write(show_tokenizations(string=sent1))
        f.write(show_tokenizations(string=sent2))
        
        
        # f.write([show_tokenizations(string=sent) for sent in cb_test[0]])
        # f.write([show_tokenizations(string=sent) for sent in cb_test[1]])