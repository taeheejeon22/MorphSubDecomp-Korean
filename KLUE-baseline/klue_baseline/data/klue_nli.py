...
class KlueNLIProcessor(DataProcessor):
    ...
    def _create_examples(self, file_path: str, dataset_type: str) -> List[InputExample]:
        ...
        for data in data_lst:
            ...
            pre = " ".join(self.pretokenizer.tokenize(pre.strip()))  ### our ### pretokenization
            hyp = " ".join(self.pretokenizer.tokenize(hyp.strip()))  ### our ### pretokenization
        ...
        for i in range(5):
            ...
            logger.info("origin example: %s" % data_lst[i]["premise"])
            logger.info("pretokenized example: %s" % self.pretokenizer.tokenize(data_lst[i]["premise"]))
